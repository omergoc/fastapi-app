...
_JWT_SECRET = "AJDJAJDLAKSJDJAKLasdsdaASDASDADS"
...
async def create_user(user: _schemas.UserRequest, db:_orm.Session):
    ...
    #create the user model to be saved database
...
async def login(email:str, password:str, db:_orm.Session):
    ...
    # Return False ıf no user wıth email found
    if not db_user:
        return False

    # Return False If No User Wıth Password
